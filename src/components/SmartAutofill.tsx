...
export default function SmartAutofill() {
    ...
    return (
        <div className="fixed right-6 top-32 z-50 w-80 bg-card border border-border/50 shadow-2xl rounded-2xl overflow-hidden flex flex-col h-[600px] max-h-[80vh] animate-in slide-in-from-right-8 duration-300">
                ...
                {loading ? (
                    ...
                    filteredProducts.length > 0 ? filteredProducts.map(p => (
                        <button key={p.id} onClick={() => addProduct(p)} className="w-full text-left p-3 rounded-xl hover:bg-indigo-50 border border-transparent hover:border-indigo-100 transition-colors group flex items-center justify-between">
                            <div className="pr-2">
                                <p className="font-bold text-sm text-foreground group-hover:text-indigo-900 line-clamp-1">{p.name}</p>
                                <p className="text-xs font-mono text-indigo-600 font-bold mt-0.5">{p.price.toLocaleString()} XOF</p>
                            </div>
