...
import { Users, Plus, Search, Edit, Trash2, Mail, Phone, MapPin, Download } from "lucide-react";
import {
    Dialog,
    DialogContent,
    DialogDescription,
...
export default function ClientsPage({ isComponent }: { isComponent?: boolean } = {}) {
    const { t } = useLanguage();
    ...
    const handleDelete = async (id: string, name: string) => {
        if (!confirm(`Toutes les factures associées à ${name} perdront la liaison directe. Voulez-vous vraiment supprimer ce client ?`)) return;
        try {
            const userStr = localStorage.getItem("user");
            if (!userStr) return;
            const user = JSON.parse(userStr);
    ...
    const resetForm = () => {
        setEditingId(null);
        setFormData({
            name: "", email: "", phone: "", address: "", contact: "", poBox: "",
            firstName: "", city: "", country: "", companyName: "", jobTitle: "",
            type: "particulier", status: "actif", preferredPaymentMethod: "cash", notes: ""
        });
    };
                        ...
                        <Dialog open={isDialogOpen} onOpenChange={(open) => {
                            ...
                            <DialogContent className="sm:max-w-[600px] bg-card border border-border/50 text-foreground backdrop-blur-xl">
                                <DialogHeader>
                                    ...
                                    <DialogDescription>
                                        Entrez les détails du client pour l'ajouter à votre CRM.
                                    </DialogDescription>
                                </DialogHeader>
                                <form onSubmit={handleSave} className="space-y-4 py-4">
                                    <div className="md:col-span-2 space-y-4">
                                        <div className="space-y-4 pt-2">
                                            <h4 className="text-xs font-black uppercase tracking-widest text-primary border-b border-border/10 pb-2">Informations Personnelles</h4>
                                        ...
                                        <div className="space-y-4 pt-4">
                                            <h4 className="text-xs font-black uppercase tracking-widest text-primary border-b border-border/10 pb-2">Informations Professionnelles</h4>
                                            <div className="grid md:grid-cols-2 gap-4">
                                                <div className="space-y-2">
                                                ...
                                                <div className="space-y-2">
                                                    <Label htmlFor="jobTitle">Profession ou secteur d’activité</Label>
                                                    <Input
                                                        id="jobTitle"
                                        ...
                                        <div className="space-y-4 pt-4">
                                            <h4 className="text-xs font-black uppercase tracking-widest text-primary border-b border-border/10 pb-2">Informations Commerciales et Paiement</h4>
                                            <div className="grid md:grid-cols-2 gap-4">
                                                <div className="space-y-2">
                                                    ...
                                                    <select
                                                        ...
                                                        className="w-full bg-muted/10 border-border/50 rounded-md h-10 px-3 text-sm focus:ring-primary/20 focus:border-primary outline-none"
                                                    >
                                        ...
                                        <div className="space-y-4 pt-4">
                                            <h4 className="text-xs font-black uppercase tracking-widest text-primary border-b border-border/10 pb-2">Commentaires</h4>
                                            <div className="space-y-2">
                                                ...
                                                <textarea
                                                    ...
                                                    rows={3}
                                                    className="w-full bg-muted/10 border-border/50 rounded-md p-3 text-sm focus:ring-primary/20 focus:border-primary outline-none resize-none"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-fade-in-up delay-200">
                    {isLoading ? (
                        ...
                        filteredClients.map((client) => (
                            <Card key={client.id} className="group bg-card border border-border/40 shadow-sm hover:shadow-md hover:border-primary/20 transition-all duration-300 rounded-2xl overflow-hidden flex flex-col h-full">
                                    ...
                                    <div className="flex justify-between items-start">
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div className="space-y-0.5">
                                                ...
                                                <div className="inline-flex items-center px-1.5 py-0.5 rounded-md bg-muted text-[10px] font-bold uppercase tracking-wider text-muted-foreground">
                                                    {client.type || "Particulier"}
                                                </div>
                                            </div>
                                        </div>
                                        <div className="flex gap-1.5">
                                            <Button variant="ghost" size="icon" className="h-8 w-8 rounded-lg text-muted-foreground hover:text-primary hover:bg-primary/10 transition-colors" onClick={() => openEdit(client)}>
                                                <Edit className="w-4 h-4" />
                                            </Button>
                                            <Button variant="ghost" size="icon" className="h-8 w-8 rounded-lg text-muted-foreground hover:text-destructive hover:bg-destructive/10 transition-colors" onClick={() => handleDelete(client.id, client.name)}>
                                    ...
                                    <div className="pt-5 border-t border-border/10">
                                        <div className="grid grid-cols-2 gap-4 mb-4">
                                            <div className="space-y-1">
                                                <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Total Payé</span>
                                                <p className="text-lg font-black text-primary tracking-tight">
                                                    {(client.totalSpent || 0).toLocaleString()} <span className="text-[10px] opacity-70">CFA</span>
                                                </p>
                                            </div>
                                            <div className="space-y-1 text-right">
                                                <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Factures</span>
                                                <p className="text-lg font-black text-foreground tracking-tight">
                                                    {client.paidInvoicesCount || 0} <span className="text-[10px] font-bold text-muted-foreground uppercase">Payées</span>
                                                </p>
                                            </div>
                                        </div>
                                        <div className="flex items-center justify-between px-3 py-2 rounded-xl bg-orange-500/5 border border-orange-500/10">
                                            <span className="text-[10px] font-bold text-orange-600 uppercase tracking-widest">En attente</span>
