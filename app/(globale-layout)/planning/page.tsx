  ...
  const fetchTodos = async (token: string) => {
    try {
      ...
      if (response.ok) {
        ...
        // Convert ISO strings back to local format if needed or keep as is
        setTodos(data);
      }
    } catch (error) {
      ...
      if (response.ok) {
        ...
        if (taskToDelete) {
          addNotification({
            user: "Système",
            action: "a supprimé",
            target: `la tâche "${taskToDelete.title}"`,
            type: "system",
          });
        }
      }
